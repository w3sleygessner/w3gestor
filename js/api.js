...
export function sendManualWA(cliId, type) {
    ...
    // Agora o sistema puxa o template correto configurado para cada alerta
    if (type === 'oscilacao') {
        template = db.config.msg_oscilacao || "⚠️ *Aviso de Instabilidade*\nOlá {cliente}, identificamos uma oscilação no servidor do app {app}. A nossa equipe já está atuando para normalizar o sinal.";
    }
    if (type === 'manutencao') {
        template = db.config.msg_manutencao || "🔧 *Aviso de Manutenção*\nOlá {cliente}, informamos que o servidor do app {app} entrará em manutenção programada para melhorias.";
    }
